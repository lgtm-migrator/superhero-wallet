...
export default defineComponent({
  ...
  setup({ transaction }) {
    ...
    const getTxTipUrl = computed(() => store.getters.getTxTipUrl);

    const getExplorerPath = computed(() => store.getters.getExplorerPath);

    const tipUrl = computed(() => getTxTipUrl.value(transaction));

    const isSwap = computed(() => FUNCTION_TYPE_DEX.swap.includes(transaction?.tx?.function || ''));

    const isPool = computed(() => FUNCTION_TYPE_DEX.pool.includes(transaction?.tx?.function || ''));
