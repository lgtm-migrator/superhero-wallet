...
export default defineComponent({
  ...
  setup(props, { root, emit }) {
    ...
    const formModel = ref<any>(props.transferData);
    ...
    watch(
      ...
      (val) => emit('input', {
        ...val,
        fee: fee.value,
        total: (val.selectedAsset && val.selectedAsset.contractId === AETERNITY_CONTRACT_ID
    ...
    const queryHandler = async (query:any) => {
      if (query.token && availableTokens.value[query.token]) {
    ...
    const openScanQrModal = async () => {
      ...
      if (scanResult?.trim().charAt(0) === '{') {
        ...
        if (!requestedTokenBalance) {
          formModel.value.address = '';
          root.$store.dispatch('modals/open', { name: 'default', type: 'insufficient-balance' });
          formModel.value.address = '';
          return;
        }
